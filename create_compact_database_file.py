            ...
            for dump in elem.findall('dump'):
                for rom in dump.findall('rom'):
                    ...
                    if controls_elem is not None:
                        controls = int(controls_elem.text)
                    else:
                        # Find controls in parent <software> element
                        controls_elem = elem.find('controls')
                        if controls_elem is not None:
                            controls = int(controls_elem.text)
                    rom_data_list.append((hash, mapper, controls, ctrl))
...
def write_to_binary(rom_data_list, output_file):
    with open(output_file, 'wb') as outfile:
        for sha1string, mapper, controls, ctrl in sorted(rom_data_list):
            sha1_bytes = bytes.fromhex(sha1string)
            if len(sha1_bytes) == 20:
                outfile.write(sha1_bytes[:6])
                outfile.write(struct.pack("B", mapper))
                outfile.write(struct.pack("B", controls))
                outfile.write(struct.pack("B", ctrl))
#            else:
#                print("Ignored incorrect sha1 : ",sha1string)

def main(xml_file, output_file):
    rom_data_list = extract_rom_data(xml_file)
    write_to_binary(rom_data_list, output_file)

if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python create_compact_database_file.py <xml file> <compact bin file>")
        sys.exit(1)

    xml_file = sys.argv[1]
    bin_file = sys.argv[2]

    main(xml_file, bin_file)
